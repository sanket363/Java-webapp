  ...
  // Test Java application
   ...
   // Pull docker image from DockerHub and run in EC2 instance 
    ...
    stage('Deploy Docker image to AWS instance') {
	  ...
	  sh "ssh -o StrictHostKeyChecking=no ${REMOTE_USER}@${REMOTE_SERVER} 'docker pull snaket2628/javawebapp'"
          sh "ssh -o StrictHostKeyChecking=no ${REMOTE_USER}@${REMOTE_SERVER} 'docker run --name javaApp -d -p 8081:8081 snaket2628/javawebapp'"
          }
        }
      }
    }
